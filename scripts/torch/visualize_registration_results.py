...
def _show_current_images_1d(iS, iT, iW, iter, vizImage, vizName, phiWarped,visual_param=None, i=0):
    ...
    plt.subplot(sp_w)
    plt.plot(t2np(iT),'g',linestyle=':')
    plt.plot(t2np(iS),'r',linestyle='--')
    plt.plot(t2np(iW),linestyle=':')
    plt.title('warped image')

    if phiWarped is not None:
        plt.subplot(sp_p)
        plt.plot(t2np(phiWarped[0,:]))
        plt.title('phi')

    if vizImage is not None:
        plt.subplot(sp_v)
        plt.plot(utils.lift_to_dimension(t2np(vizImage),1))
        plt.title(vizName)
...
def _show_current_images_2d_no_map(iS, iT, iW, iter, vizImage, vizName, visual_param=None, i=0):
    ...
    plt.subplot(sp_s)
    plt.imshow(t2np(iS),cmap='gray')
    plt.colorbar()
    plt.title('source image')

    plt.subplot(sp_t)
    plt.imshow(t2np(iT),cmap='gray')
    plt.colorbar()
    plt.title('target image')

    plt.subplot(sp_w)
    plt.imshow(t2np(iW),cmap='gray')
    plt.colorbar()
    plt.title('warped image')

    plt.subplot(sp_c)
    plt.imshow(checkerboard_2d(t2np(iW),t2np(iT)),cmap='gray')
    plt.colorbar()
    plt.title('checkerboard')

    if vizImage is not None:
        plt.subplot(sp_v)
        plt.imshow(utils.lift_to_dimension(t2np(vizImage),2),cmap='gray')
        plt.colorbar()
        plt.title(vizName)
...
def _show_current_images_2d_map(iS, iT, iW,iSL,iTL, iWL, iter, vizImage, vizName, phiWarped, visual_param=None, i=0):
    ...
    if phiWarped is not None:
        ...
        plt.contour(t2np(phiWarped[0, :, :]), np.linspace(-1, 1, 20), colors='r', linestyles='solid',
                    linewidths=0.5)
        plt.contour(t2np(phiWarped[1, :, :]), np.linspace(-1, 1, 20), colors='r', linestyles='solid',
                    linewidths=0.5)
...
def _show_current_images_3d(iS, iT, iW,iSL, iTL,iWL, iter, vizImage, vizName, phiWarped, visual_param=None, i=0,extraImage=None, extraName= None):
    ...
    ivsx = viewers.ImageViewer3D_Sliced(ax[0][0], t2np(iS), 0, 'source X', True)
    ivsy = viewers.ImageViewer3D_Sliced(ax[0][1], t2np(iS), 1, 'source Y', True)
    ivsz = viewers.ImageViewer3D_Sliced(ax[0][2], t2np(iS), 2, 'source Z', True)

    ivtx = viewers.ImageViewer3D_Sliced(ax[1][0], t2np(iT), 0, 'target X', True)
    ivty = viewers.ImageViewer3D_Sliced(ax[1][1], t2np(iT), 1, 'target Y', True)
    ivtz = viewers.ImageViewer3D_Sliced(ax[1][2], t2np(iT), 2, 'target Z', True)

    ivwx = viewers.ImageViewer3D_Sliced(ax[2][0], t2np(iW), 0, 'warped X', True)
    ivwy = viewers.ImageViewer3D_Sliced(ax[2][1], t2np(iW), 1, 'warped Y', True)
    ivwz = viewers.ImageViewer3D_Sliced(ax[2][2], t2np(iW), 2, 'warped Z', True)

    if phiWarped is not None:
        ivwxc = viewers.ImageViewer3D_Sliced_Contour(ax[phiw_a][0], t2np(iW), t2np(phiWarped), 0, 'warped X', True)
        ivwyc = viewers.ImageViewer3D_Sliced_Contour(ax[phiw_a][1], t2np(iW), t2np(phiWarped), 1, 'warped Y', True)
        ivwzc = viewers.ImageViewer3D_Sliced_Contour(ax[phiw_a][2], t2np(iW), t2np(phiWarped), 2, 'warped Z', True)

    if vizImage is not None:
        ivvxc = viewers.ImageViewer3D_Sliced_optical_flow(ax[vizi_a][0], t2np(vizImage), 0, vizName + ' X', False)
        ivvyc = viewers.ImageViewer3D_Sliced_optical_flowd(ax[vizi_a][1], t2np(vizImage), 1, vizName + ' Y', False)
        ivvzc = viewers.ImageViewer3D_Sliced_optical_flow(ax[vizi_a][2], t2np(vizImage), 2, vizName + ' Z', False)
    if extraImage is not None:
        # ivexc = viewers.ImageViewer3D_optical_flow_Sliced(ax[ext_a][0], t2np(extraImage), 0, extraName + ' X', False)
        # iveyc = viewers.ImageViewer3D_optical_flow_Sliced(ax[ext_a][1], t2np(extraImage), 1, extraName + ' Y', False)
        # ivezc = viewers.ImageViewer3D_optical_flow_Sliced(ax[ext_a][2], t2np(extraImage), 2, extraName + ' Z', False)
        ivexc = viewers.ImageViewer3D_Sliced(ax[ext_a][0], utils.lift_to_dimension(t2np(extraImage[0]), 3), 0, extraName + ' X', True)
        iveyc = viewers.ImageViewer3D_Sliced(ax[ext_a][1], utils.lift_to_dimension(t2np(extraImage[1]), 3), 1, extraName + ' Y', True)
        ivezc = viewers.ImageViewer3D_Sliced(ax[ext_a][2], utils.lift_to_dimension(t2np(extraImage[2]), 3), 2, extraName + ' Z', True)
        # ivexc = viewers.ImageViewer3D_Sliced(ax[ext_a][0], t2np(extraImage), 0, extraName + ' X', True)
        # iveyc = viewers.ImageViewer3D_Sliced(ax[ext_a][1], t2np(extraImage), 1, extraName + ' Y', True)
        # ivezc = viewers.ImageViewer3D_Sliced(ax[ext_a][2], t2np(extraImage), 2, extraName + ' Z', True)

    if iSL is not None and iTL is not None:
        ivslxc = viewers.ImageViewer3D_Sliced(ax[iSL_a][0], utils.lift_to_dimension(t2np(iSL), 3), 0,
                                              'Lsource X', True, True)
        ivslyc = viewers.ImageViewer3D_Sliced(ax[iSL_a][1], utils.lift_to_dimension(t2np(iSL), 3), 1,
                                              'Lsource Y', True, True)
        ivslzc = viewers.ImageViewer3D_Sliced(ax[iSL_a][2], utils.lift_to_dimension(t2np(iSL), 3), 2,
                                              'Lsource Z', True, True)
        ivtlxc = viewers.ImageViewer3D_Sliced(ax[iTL_a][0], utils.lift_to_dimension(t2np(iTL), 3), 0,
                                              'LTarget X', True, True)
        ivtlyc = viewers.ImageViewer3D_Sliced(ax[iTL_a][1], utils.lift_to_dimension(t2np(iTL), 3), 1,
                                              'LTarget Y', True, True)
        ivtlzc = viewers.ImageViewer3D_Sliced(ax[iTL_a][2], utils.lift_to_dimension(t2np(iTL), 3), 2,
                                              'LTarget Z', True, True)
        ivwlxc = viewers.ImageViewer3D_Sliced(ax[iWL_a][0], utils.lift_to_dimension(t2np(iWL), 3), 0,
                                              'LWarped X', True, True)
        ivwlyc = viewers.ImageViewer3D_Sliced(ax[iWL_a][1], utils.lift_to_dimension(t2np(iWL), 3), 1,
                                              'LWarped Y', True, True)
        ivwlzc = viewers.ImageViewer3D_Sliced(ax[iWL_a][2], utils.lift_to_dimension(t2np(iWL), 3), 2,
                                              'LWarped Z', True, True)
